...
Copy `.env.example` to `.env` and fill in any optional tokens (e.g. `VITE_POSTHOG_PROJECT_TOKEN` for analytics). Vite auto-loads `.env` at build/dev time. The `.env` file is gitignored.
