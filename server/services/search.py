"""Search aggregation business logic.

Queries conversations, projects, artifacts, attachments, schedules, and pins
and returns scored results.
"""
from __future__ import annotations

from anton_api import conversation_manager
from routes.artifacts import list_artifacts
from routes.cowork_state import load_state
from routes.projects import list_projects
from services.attachments import get_attachments
...
async def search_all(query: str, limit: int = 25) -> list[dict]:
    if not query.strip():
        return []
    ...
    for conv in conversations:
        ...
        text = " ".join([conv.get("title") or "", conv.get("preview") or "", project_label])
    ...
    # Schedules + Pins (from cowork state)
    state = load_state()
