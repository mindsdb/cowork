...
export interface MindsOrg {
   ...
   *  one, which defaults to `Personal Organization` for a personal org. */
  displayName: string;
 ...
 * hand. Only the membership listing carries Keycloak's display name, so readers
 * that run before or without it fall back here rather than to
 * `personal_<uuid>` or to nothing.
 *
...
const AUTH_PERSONAL_ORG_DISPLAY_NAME = 'Personal Organization';
 ...
 * organization: Keycloak's generated label is normalized to
 * `PERSONAL_ORG_LABEL`, while a user-chosen display name stays visible.
 *
 ...
 *
 * The generated forms are blank, the raw `personal_<uuid>` slug, auth's current
 * `Personal Organization` label, its earlier `Personal` fallback, and every
...
function isGeneratedPersonalOrgName(org: MindsOrg): boolean {
  ...
  if (current === AUTH_PERSONAL_ORG_DISPLAY_NAME) return true;
 ...
 * label of our own. Auth owns the generated display-name rule, and rebuilding
 * it here would create another copy that can drift on its own schedule.
 *
