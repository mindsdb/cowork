...
//
// House rules: React 19 hooks, no new deps, inline styles + CSS vars.
 ...
 * @param {Function} [props.previewArtifact]   Override the fetcher (tests). Defaults
 *                                              to api.js previewArtifact.
 */
export function VersionDiff({
  ...
  const fetcher = previewArtifact || apiPreviewArtifact;

  const from = fromVersion || { id: '', n: '?', label: '' };
  const to = toVersion || { id: '', n: '?', label: 'current' };
  ...
  useEffect(() => {
    ...
    if (!path || !from.id) {
      setState({
        ...
        oldText: '',
        newText: '',
        error: 'Choose a saved version to compare.',
      });
      return undefined;
    }
    ...
    Promise.all([
      ...
      Promise.resolve(fetcher(path, { versionId: to.id || undefined })),
    ])
      .then(([a, b]) => {
  ...
  const toLabel = to.label === 'current'
    ? `v${to.n} · current`
