...
const missingProposeEdit = () => Promise.reject(new Error('AI edit is unavailable for this artifact.'));
const missingCommitEdit = () => Promise.reject(new Error('Artifact edit persistence is unavailable.'));
...
export function useInlineEdit({
  proposeEdit = missingProposeEdit,
  commitEdit = missingCommitEdit,
  onComment,
  onCommitted,
  baseVersionId,
} = {}) {
  ...
  // The version the next commit is computed against. Starts at the prop, but a
  // 409 conflict advances it to the server's current version so a subsequent
  // "Merge & keep" re-applies against the NEW base (a client-side rebase)
  // rather than re-posting the same stale base and 409-ing forever.
  const baseVersionRef = useRef(baseVersionId);
  useEffect(() => { baseVersionRef.current = baseVersionId; }, [baseVersionId]);
  ...
  const keep = useCallback(async () => {
    ...
    const target = s.target;
    try {
      const res = await commitEdit({ target, newText, baseVersionId: baseVersionRef.current });
      if (!mountedRef.current || myReq !== reqIdRef.current) return;
      ...
      if (isConflict) {
        // Advance the base to the server's current version so the next
        // "Merge & keep" re-applies against the NEW base instead of looping.
        const nextBase = res?.conflict?.currentVersionId ?? res?.currentVersionId ?? null;
        if (nextBase) baseVersionRef.current = nextBase;
        dispatch({
      ...
      if (err?.status === 409 || err?.code === 'conflict') {
        const nextBase = err?.currentVersionId ?? err?.conflict?.currentVersionId ?? null;
        if (nextBase) baseVersionRef.current = nextBase;
        dispatch({
          type: 'COMMIT_CONFLICT',
          conflict: {
  ...
  }, [s.state, s.diff, s.target, commitEdit, onCommitted]);
