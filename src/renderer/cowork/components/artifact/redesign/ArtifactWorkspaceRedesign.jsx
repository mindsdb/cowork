// ArtifactWorkspaceRedesign.jsx — redesigned artifact workspace.
//
...
//
// Versions/comments come from api.js; the Story feed is built in railEvents
// (versions + comments interleaved by time, live chat appended).
...
export const REDESIGN_FLAG_KEY = 'anton:artifact-workspace-redesign';
...
function ProseCanvas({
  ...
  // M1 adapters: inject `path` + the block's `oldText` and hit the real backend.
  const proposeEdit = useCallback(
    async ({ target, instruction }) => {
      ...
      if (!res || (!res.newText && !res.oldText)) {
        throw new Error('AI edit returned no proposed change.');
      }
  ...
  const commitEdit = useCallback(
    async ({ target, newText, baseVersionId: bv }) => {
      return acceptArtifactEdit({ path, target, newText, baseVersionId: bv });
    },
    [path],
  );
...
export function ArtifactWorkspaceRedesign({
  ...
  const handleCommitted = useCallback(
    ({ versionId } = {}) => {
      if (!versionId) {
        flash('AI editing did not create a saved version.');
        bumpReload();
        return;
      }
