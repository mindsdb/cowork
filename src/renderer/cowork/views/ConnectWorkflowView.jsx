...
const PLUGIN_DIRECTORY_CARDS = [
  { name: 'Box', vendor: 'Box', desc: 'Work with your Box content directly from Minds - search files, organize folders, collaborate with your team.' },
...
function ConnectorsPage({
  ...
  if (driveSelected) {
    ...
    detailNotes.push(`Setup for ${selectedConnector?.name} is not wired into Minds yet.`);
  }
        ...
        <div className="customize-empty-detail">
          ...
          <div className="customize-empty-title">
            {selectedConnector?.status === 'included'
              ? `${selectedConnector.name} is already available in Minds.`
              : connectMessage}
          </div>
          <button
