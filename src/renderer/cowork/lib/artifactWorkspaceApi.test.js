...
// Short legacy ids are unaddressable; fail to preview rather than sending an invalid workspace
// request.
...
/* Org text drafts stream uncapped; the client must cap preview text and report truncation. */
describe('loadArtifactDraftText', () => {
  ...
  /* Preserve numeric status on errors so the viewer can choose access-specific copy. */
  ...
  /* Embedded/cross-origin text remains readable through an uncredentialed fetch. */
  ...
  /* Report embedded content explicitly rather than misclassifying it as a malformed URL. */
...
/* HTML srcdoc requires the full document; the text preview's 200k cap would silently break it. */
describe('loadArtifactDraftDocument', () => {
  ...
  /* Credential attachment must enforce same-origin itself even if the viewer already gates callers. */
  it('refuses to attach credentials to a cross-origin draft URL', async () => {
    authFetch.mockClear();
