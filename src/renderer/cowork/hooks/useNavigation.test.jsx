...
describe('useNavigation', () => {
  ...
  it('navigate(key) flips the route, mirrors it into routeRef, and returns to the Cowork workspace', () => {
    ...
    expect(deps.setWorkspaceMode).toHaveBeenCalledWith('cowork');
  });

  it('routeRef reports "code" while the Code workspace is active', () => {
    const { result } = render({ effectiveWorkspaceMode: 'code' });
    expect(result.current.routeRef.current).toBe('code');
    act(() => result.current.navigate('artifacts'));
    // Route state still flips underneath; routeRef stays code-scoped for Cmd+B.
    expect(result.current.route).toBe('artifacts');
    expect(result.current.routeRef.current).toBe('code');
  });
  ...
  it('navigate("code") switches to the Code workspace instead of routing', () => {
    const { result, deps } = render();
    act(() => result.current.navigate('code'));
    expect(deps.openCode).toHaveBeenCalled();
    expect(result.current.route).toBe('home'); // unchanged
    expect(deps.setWorkspaceMode).not.toHaveBeenCalled();
  });
