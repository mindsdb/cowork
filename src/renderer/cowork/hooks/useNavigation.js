...
// open, the Code workspace switch, the tasks/projects/artifacts stores, the
...
// their many consumer sites through this return. `effectiveWorkspaceMode` is
// injected so `routeRef` reports `code` while the Code workspace is active,
// keeping the Cmd+B collapse scope correct without the hook owning Code state.
export function useNavigation({
  ...
  // mount) can read it without a re-bind on every nav. Reports `code` while the
  // Code workspace is active so Cmd+B collapse treats it as one nav scope.
