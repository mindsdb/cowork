...
describe('useModels', () => {
  ...
  it('derives models from recommendedModelOptions for the planning provider', () => {
    ...
    expect(result.current.mindsModels).toBe(result.current.models);
  });

  it('keeps MindsHub models available when the planning provider is direct', () => {
    providerValueToType.mockReturnValue('anthropic');
    recommendedModelOptions
      .mockReturnValueOnce([{ id: 'air', label: 'MindsHub Air' }])
      .mockReturnValueOnce([{ id: 'sonnet', label: 'Claude Sonnet' }]);

    const { result } = render({ ...baseSettings, planningProvider: 'anthropic' });

    expect(result.current.mindsModels).toEqual([{ id: 'air', name: 'MindsHub Air' }]);
    expect(result.current.models).toEqual([{ id: 'sonnet', name: 'Claude Sonnet' }]);
  });
  ...
  it('modelMeta exposes the provider/family/enabled maps and wires onRefresh', () => {
    ...
    expect(result.current.modelMeta.modelEfforts).toBe(baseSettings.modelEfforts);
    expect(result.current.modelMeta.harness).toBe(baseSettings.harness);
  ...
  it('refreshModelAvailability merges a fresh fetch into settings', async () => {
    ...
    expect(mergeRecommendedModels).toHaveBeenCalledWith(
      baseSettings,
      { recommendedModels: { x: 1 } },
      { keepOrder: true },
    );
