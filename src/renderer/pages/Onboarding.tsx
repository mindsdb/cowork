...
export default function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  const handleConnect = async () => {
    ...
    const result = await window.antontron.validateProvider(
      provider,
      apiKey.trim(),
      provider === 'minds' ? mindsUrl.trim() : undefined
    );
  ...
  █▀█ █ ▀█  █  █▄█ █ ▀█`}</pre>
      ...
      <div className="provider-cards">
        <button
          ...
          onClick={() => { setProvider('minds'); setPhase('choose'); setErrorMsg(''); }}
        >
          <div className="provider-card-name">Minds</div>
          <div className="provider-card-desc">MindsDB Platform</div>
        </button>
        <button
          className={`provider-card ${provider === 'anthropic' ? 'selected' : ''}`}
          onClick={() => { setProvider('anthropic'); setPhase('choose'); setErrorMsg(''); }}
        >
          <div className="provider-card-name">Anthropic</div>
          <div className="provider-card-desc">Claude API</div>
        </button>
      </div>
      ...
      <div className="onboard-fields">
        ...
        <div className="onboard-field">
          <label className="onboard-label">
            {provider === 'anthropic' ? 'Anthropic API Key' : 'Minds API Key'}
          </label>
          <input
            type="password"
            className="settings-input"
            placeholder={provider === 'anthropic' ? 'sk-ant-...' : 'Your Minds API key'}
            value={apiKey}
