...
- The sidebar draws no MindsHub workspace selector. `components/WorkspaceSelector.jsx`,
  `hooks/useHubWorkspaces.js` and `lib/letterTile.js` are retained, tested and pinned, but
  nothing mounts them, so the renderer never reads `/api/v1/hub/workspaces/`. Auth's
  `authorization_ui` Statsig gate still governs the console and is unchanged; Cowork stopped
  reading it rather than anyone switching it off. Details in the README under "The workspace
  selector, which the sidebar does not draw".
